...
**T2I Diffusion Finetuning.** We efficiently fine-tune a text-to-image diffusion model on fundus data using a DreamBooth-style framework, where $\mathrm{MSE}*{\text{DDR}}$ serves as the subject-instance loss and $\mathrm{MSE}*{\text{prior}}$ as the class-specific prior preservation loss, enabling stable adaptation to the fundus domain.
